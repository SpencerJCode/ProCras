...
const Study = (props) => {
  ...
  return (
    <div>
      <div className="d-flex col-11 justify-content-around m-auto mt-3">
          ...
          <form onSubmit={handleSubmit}>
            <div className="card my-shadow selection-list">
              ...
              <div className="card-body">
                <h6 className="text-center text-secondary">Repeated cards will not be included in study session.</h6>
                <div>
                  {selectedStacks.map((stack, i) => {
                    return (
                      <div className="d-flex align-items-center justify-content-between m-2">
                        <p>{stack.stackName}</p>
                        <button className="btn btn-danger" onClick={(e) => removeFromList(e, stack)}>Remove Stack</button>
                      </div>
                    )
                  })}
                  {selectedDecks.map((deck, i) => {
                    return (
                      <div className="d-flex align-items-center justify-content-between m-2">
                        <p>{deck.deckName}</p>
                        <button className="btn btn-danger" onClick={(e) => removeFromList(e, deck)}>Remove Deck</button>
                      </div>
                    )
                  })}
                </div>
          ...
          <div className="card-body">
            {sessionDecks.map((deck, i) => {
              ...
              return <div className="d-flex align-items-center justify-content-between m-2">
                <p>{deck.deckName}</p>
                <p>{successRate}</p>
                <button className="btn btn-danger" onClick={(e) => removeFromList(e, deck)}>Remove Session</button>
              </div>
            })}
          </div>
        </div>
